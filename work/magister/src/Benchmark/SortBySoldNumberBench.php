...
class SortBySoldNumberBench extends AbstractBenchmark
{
    ...
    public function getRows3NF(): array
    {
        $results = $this->nf->executeQuery("
            ...
            ORDER BY sold_count DESC, p.product_id
            ;
        ")->fetchAll();
    ...
    public function getRowsNon3NF(): array
    {
        $results = $this->non3nf->executeQuery("
            SELECT
                ...
                sold_count
            FROM non3nf
            ORDER BY sold_count DESC, product_id
            ;
        ")->fetchAll();
    ...
    {
        return "Wszystkie produkty, posortowane po ilosci sprzedanych sztuk - produkty najczesciej kupowane na poczatku (bez indeksow na sortowana kolumne)";
    }
