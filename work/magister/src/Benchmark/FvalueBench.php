...
class FvalueBench extends AbstractBenchmark
{

    const FVALUE_IDS = "11, 22, 33, 44, 55, 66";
    ...
    public function getRows3NF(): array
    {
        $results = $this->nf->executeQuery("
            ...
            WHERE pv.fvalue_id IN (" . self::FVALUE_IDS . ")
    ...
    public function getRowsNon3NF(): array
    {
        $results = $this->non3nf->executeQuery("
            ...
            FROM non3nf
            WHERE (
                ARRAY [11]::bigint[] <@ fvalue_ids
                OR ARRAY [22]::bigint[] <@ fvalue_ids
                OR ARRAY [33]::bigint[] <@ fvalue_ids
                OR ARRAY [44]::bigint[] <@ fvalue_ids
                OR ARRAY [55]::bigint[] <@ fvalue_ids
                OR ARRAY [66]::bigint[] <@ fvalue_ids
            )
            ORDER BY product_id
            ;
        ")->fetchAll();
    ...
    {
        return "Wszystkie produkty, ktore posiadaja cechy (wartosci) o id: " . self::FVALUE_IDS;
    }
